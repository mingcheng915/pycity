            ...
            while iteration < max_iterations and (max_time is None or start_tick + max_time > time.monotonic()):
                ...
                if mode == "release" and release_iteration>=SWITCH_COUNT:
                    ...
                    if all(all(last[0] == l
                        ...
                        #new binary solution?
                        if not any(current == e for e in explored_Binary_Solutions):
                            mode = "fix"
                            fix_nodes()
                        else:
                            rho /= RHO_FACTOR
                ...
                if r_norms[-1] <= eps_primal and s_norms[-1] <= eps_dual:
                    ...
                    for node in nodes.values():
                        obj += node['entity'].get_objective(beta).getValue()
                ...
                for node_id, node in nodes.items():
                    entity = node['entity']
                    last = last_Binary_Solutions[node_id]
                    if not isinstance(
                            entity,
                            (Building, Photovoltaic, WindEnergyConverter)
                    ):
